...
def slope_of_linear_equation(x1, y1, x2, y2):
    if x1 == x2:
...
def evens_and_odds(n):
    ...
    odd_len = len(odds)
    even_pnt = print(f'The number of even numbers is {even_len}')
    odd_pnt = print(f'The number of odd numbers is {odd_len}')
    return even_pnt, odd_pnt
...
def same_data_type(*args):
    return all(type(i) == type(args[0]) for i in args)
...
def most_spoken_languages(countries):
    ...
    most_spoken_languages = sorted(language_counts.most_common(10), ascending=true)

    # Prepare results
    most_spoken_languages_list = most_spoken_languages
    return most_spoken_langauges_list
